...
@Properties({ @Property(name = "service.description", value = "Save product Servlet"),
		...
		@Property(name = "sling.servlet.paths", value = { "/servlet/service/OutputManagementTest" }),
...
public class OutputManagement_DataTest extends SlingAllMethodsServlet {
...
//									out.println("cell.cell.getColumnIndex()306  = " );
									while (cells.hasNext()) {
										...
										if (cell.getColumnIndex() == 1) {
											...
											out.println(" value====== " + objectdata);
											out.println("cell.cell.getColumnIndex()306 value is = "
													+ cell.getColumnIndex());
										...
										if (cell.getColumnIndex() == 2) {
											...
											out.println("cell.cell.getColumnIndex()319  = " + cell.getColumnIndex()
													+ "= valuedata============-- " + valuedata);
										}
//										out.println("cell.cell.getColumnIndex()  = "+cell.getColumnIndex() );
										if (cell.getColumnIndex() == 3) {
											...
											primarykeydata = objDefaultFormat.formatCellValue(cell,
													objFormulaEvaluator);
											out.println("cell objformulaevaluator	primarykeydata 327========== = "
													+ primarykeydata);
										}
...
//											try {
												out.println("excelname.getName()364 : " + excelname.getName());
												externalrow = (HSSFRow) externalrows.next();
												out.println("externalrow.getRowNum() ==  : " + externalrow.getRowNum());
...
//											 out.println("First Cell : "+i);
												...
												try {
													while (externalcells.hasNext()) {
														out.println("Primary in while is 385***************88888=="
																+ primarykeydata);
														externalcell = (HSSFCell) externalcells.next();
														externalcellFirst = (HSSFCell) firstRowIt.next();
														// out.println("Column_Index : " +
														// externalrow.getLastCellNum());
														// if (externalrow.getRowNum() == 0) {
														// // keysvalue = objDefaultFormat.formatCellValue(cell,
														// objFormulaEvaluator);
														// externalobjFormulaEvaluator.evaluate(externalcell); // This
														// will
														// evaluate the
														...
														if (externalcell.getColumnIndex() == 0) {
															...
															// externalcell.getStringCellValue()
															out.println("Data Formatter Values 413 : " + val);
														} else {
															val = externalobjDefaultFormat.formatCellValue(
																	externalcellFirst, externalobjFormulaEvaluator);
															out.println("externalobj format :- " + val);
														}
// raw data ----------------------"1015"
														out.println("primerykey is 416========= = " + primarykeydata);
														if (val.equals(primarykeydata)) {
															out.println("valequals========== = " + val);
															iCount = 1;
															if (externalcell.getColumnIndex() < externalrow
																	.getLastCellNum()) {

																externalobjFormulaEvaluator.evaluate(externalcell);
																o = externalobjDefaultFormat.formatCellValue(
																		externalcell, externalobjFormulaEvaluator);
...
//													 out.println("mainobject 434- "+mainobject);
															...
															out.println("insidejson 440----: " + val + " pk=="
																	+ primarykeydata);
														}
										...
										out.println("exc= in excel " + e.getMessage());

									}
//									 out.println("insidejson 454--: "+insidejson);
									...
									try {
										...
										out.println("mainobject 446: lengt= " + datavalues.length + "datavalues- -"
												+ datavalues);
									...
									if (exceldata.length() > 0) {
										...
										try {
											Value[] transformRawValue = carrotmainNode.getNode(project)
													.getNode("TRANSFORM_DATA").getNode("Raw_Data")
													.getProperty("Raw_Data").getValues();
											out.println("transformRawValue 485: " + transformRawValue + "=  len= "
													+ transformRawValue.length);
											for (int i = 0; i < transformRawValue.length; i++) {
												JSONObject jsonObject = new JSONObject(
														transformRawValue[i].getString());
											...
											while (transformjsonitr.hasNext()) {
												...
												exceldata.put(
														transformkeys.replaceFirst(transformkeys.substring(0, 1),
...
//											Node =null;
										...
										out.println("url--- " + "http://35.236.213.87:8080/drools/callrules/"
												...
												if (fullrulejson.has(outputkeys.get(i))) {
													...
													if (!finaloutput.contains("null,")) {
														FileOutputStream fileOut = new FileOutputStream(
																CrRuleConstValue.StringConstant.DEFAULT_CARROT_PATH
																		.value() + "Output_Management/" + filename); //
														wb.write(fileOut);
														fileOut.close();
													}
