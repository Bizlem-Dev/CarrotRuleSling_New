...
@Properties({ @Property(name = "service.description", value = "Save product Servlet"),
		...
		@Property(name = "sling.servlet.paths", value = { "/servlet/service/FinalOutputmgmt" }),
...
//											out.println("ruleflowjsonobject 344= " + ruleflowjsonobject);
											if (carrotmainNode.getNode(project).getNode("EXTERNAL_DATA")
												...
												try {
													...
													searcheddata.remove("processInitiator");
													if (searcheddata.length() > 0) {
														seracheddatarr.put(searcheddata);
														out.println("searcheddata -----------------------------= "+seracheddatarr);
									 ...
									 rowcountinexcel=seracheddatarr.length();
									// output excel data
									if (seracheddatarr.length() > 0) {
										searcheddataALL.put("searchedMongodata", seracheddatarr);
...
//										http://35.186.166.22:8082/drools/callrules/carrotrule@xyz.com_LeadAutoConvertRules_RulesLAC/fire
											...
											String rp=	writeToFile(searcheddataALL, mainobject,username,project,out);
												
											out.println("in write--- rprprp "+rp);
	...
	public String writeToFile(JSONObject exceldata, JSONObject mainobject,String username,String project,PrintWriter out) {//HSSFWorkbook workbook,
		...
		try {
			...
			JSONArray excldataarr=exceldata.getJSONArray("searchedMongodata");
			 rowcountexcel=excldataarr.length();
			out.println("rowcountexcel= "+ rowcountexcel);
...
//			for (int rowNum = 1; rowNum <= rowcountexcel; rowNum++) {
				int columnCount = 0;
...
//						    "Addedop1": "2"}
							...
							while (keysrul.hasNext()) {
								...
								try {
									out.println("Json Data Rule Input: " + excelOutputData);//35.186.166.22:8082
									
									json = uploadToServer("http://"+bundleststic.getString("carrotRuleDroolIP")+":8082/drools/callrules/"
											+ username + "_" + project + "_"
											+ rlname + "/fire", excelOutputData);
									fireresp=new  JSONObject(json);
									out.println("Method Return : " + json);
//									out.println("in mainobj= "+fireresp);
...
//									JSONObject transjs=transar.getJSONObject(j);
						...
						out.println("excelOutputData= " + excelOutputData);
			...
			out.println("in excel num================= "+noOfColumns);
			System.out.println(
...
//			fileOut = new FileOutputStream(fp);
			fileOut = new FileOutputStream(CrRuleConstValue.StringConstant.DEFAULT_CARROT_PATH.value()
					+ "Output_Management_Consolidator/" + filePath);
			workbook.write(fileOut);
			 out.println("Generaed Excel File");
		} catch (Exception e) {
			e.printStackTrace();
		...
		return "{\"status\":\"S\",\"message\":\"http://"+bundleststic.getString("SlingIP")+":8082/carrotruleexcelfiles/Output_Management_Consolidator/"
				+ filePath + "\"}";
